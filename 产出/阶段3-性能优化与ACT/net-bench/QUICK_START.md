...
### CI 测试（明确模式）
```bash
# x86_64 vhost + KVM（主力性能测试）
cargo xtask starry app qemu \
    --test-case net-bench \
    --arch x86_64 \
    --qemu-config apps/starry/net-bench/qemu/vhost-x86_64-kvm.toml

# x86_64 slirp（功能冒烟，无需 sudo）
cargo xtask starry app qemu \
    --test-case net-bench \
    --arch x86_64 \
    --qemu-config apps/starry/net-bench/qemu/slirp-x86_64-kvm.toml

# aarch64 vhost + TCG（跨架构测试）
cargo xtask starry app qemu \
    --test-case net-bench \
    --arch aarch64 \
    --qemu-config apps/starry/net-bench/qemu/vhost-aarch64-tcg.toml
```
...
**注意：**
- `slirp` 场景使用 QEMU usermode 网络，无需 sudo 配置
- `vhost` 和 `tap` 场景需要 TAP 网络配置，需要 sudo
